...
class model2(torch.nn.Module):
    ...
    '''for:'''
    def __init__(self, pooling_size=(16/346, 12/260), pooling_outputs=32, pooling_after_conv2=False, more_layer=False, more_block=False, input_features=1, graph_type="unknown"):
        ...
        # Store graph type for debugging
    ...
    def forward(self, data):
        ...
        print(f"DEBUG [{self.graph_type}]: Final x shape: {x.shape}")
        ...
        if x.size(1) != self.fc.in_features:
            ...
            import torch
            device = next(self.parameters()).device # <---- added recently - creates new fc layer on current device
            self.fc = torch.nn.Linear(actual_input_size, 10).to(device)   #changed from: .cuda()
